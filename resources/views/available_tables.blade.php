<x-app-layout>
    <div class="site-content">
                ...
                <label for="seats">
                    ...
                    <div class="form-control-group has-icon-right">
                        <select id="seats" class="form-control unselected" name="seats">
                            <option value="{{ $oldReservation['seats'] }}" selected>
                                {{ $oldReservation['seats'] }} People</option>
                ...
                <label label for="time">
                    ...
                    <div class="form-control-group has-icon-left has-icon-right">
                        ...
                        <select id="time" class="form-control unselected" name="time">
                            <option value="{{ $oldReservation['value_time'] }}" selected>
                                {{ $oldReservation['time'] }}</option>
        ...
        {{-- @foreach ($tableData as $table)
            <p>{{ $table['name'] }}</p>
            <p>{{ \Carbon\Carbon::parse($table['res_hour'])->format('H:i') }}</p>
        @endforeach --}}
        @if (!$uniqueResHours)
            <div class="container-sm" style="text-align: center;">
                <span class="input-label" aria-hidden="true"
                    style="color: #1C2C59 !important; margin-bottom: 50px;">There are no empty tables
                </span>
            </div>
        @else
            <div class="container-sm" style="text-align: center;">
                <span class="input-label" aria-hidden="true"
                    style="color: #1C2C59 !important; margin-bottom: 50px;">Please choose a suitable timeline
                </span>
            </div>
            <div class="container-sm">
                @foreach ($uniqueResHours as $uniqueResHour)
                    <a class="btn btn-brand"
                        href="/confirm-reservation/{{ \Carbon\Carbon::parse($uniqueResHour)->format('H:i') }}"
                        style="margin-left: 5px; margin-right: 5px; margin-top:10px;">{{ \Carbon\Carbon::parse($uniqueResHour)->format('H:i') }}</a>
                @endforeach
            </div>
        @endif
        <script>
