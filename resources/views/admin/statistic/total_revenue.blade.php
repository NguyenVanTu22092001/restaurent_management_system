<x-admin-layout>
    ...
    <div x-data="app()" x-cloak class="py-12">
        <div class=" justify-start m-2 p-2">
            <form method="POST" class="flex" action="/admin/statistic/total-revenue">
                @csrf
                <x-text-input id="year" class="block w-30 mx-4" type="number" name="year"
                    value="{{ $year }}" required autofocus autocomplete="year" />
        ...
        <div class="flex flex-col">
            ...
            <div class="shadow p-6 rounded-lg bg-white">
                <div class="md:flex md:justify-between md:items-center">
                    <div>
                        <h2 class="text-xl text-gray-800 font-bold leading-tight">TOTAL REVENUE</h2>
                        <p class="mb-2 text-gray-600 text-sm">Annual</p>
                    </div>
    ...
    <script>
        function app() {
            return {
                // chartData: [112, 10, 225, 134, 101, 80, 50, 100, 200, 0, 0, 100],
                chartData: [
                    @for ($month = 1; $month <= 12; $month++)
                        {!! json_encode($totalRevenue[$year][$month]) !!},
                    @endfor
                ],
